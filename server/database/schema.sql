...
VALUES (
  ...
  "Pour l'organisation de ce projet, nous avons adopté les méthodologies Scrum et Agile, afin de structurer efficacement notre travail en équipe. Chaque membre de l’équipe avait un rôle bien défini pour assurer le bon déroulement du projet. Dylan Robinaud a pris le rôle de Product Owner, responsable de définir les fonctionnalités du produit et de prioriser les tâches. Imanol Delporte a assuré la fonction de Scrum Master, garantissant que les pratiques Scrum étaient respectées, facilitant la communication au sein de l'équipe et veillant à la résolution des obstacles.
...
Nous avons également réalisé une maquette poussée du site sur Figma, définissant clairement l'interface utilisateur et l'apparence générale du projet. En parallèle, nous avons conçu un parcours utilisateur complet, toujours sur Figma, pour illustrer le cheminement des utilisateurs à travers les différentes fonctionnalités et pages de l’application.
Pour structurer le projet, nous avons mis en place un backlog détaillé, incluant des user stories pour chaque fonctionnalité à développer. Ces user stories incluaient une description des attentes, les critères d’acceptation, et une priorisation afin d’organiser efficacement les sprints.
En ce qui concerne la modélisation de la base de données, nous avons utilisé DB Diagram pour concevoir une base de données relationnelle comprenant plus de 15 tables. Nous avons suivi les principes de la méthode Merise pour organiser les différentes entités et leurs relations, garantissant ainsi une structure de données solide et évolutive.
Enfin, pour assurer le suivi et la coordination de notre travail, nous avons utilisé Google Sheets comme support principal. Ce document nous a permis de maintenir une vision claire et partagée de l’avancement du projet, en suivant les différentes tâches, leur statut, et les deadlines associées.",
...
Le jeu est composé de 3 combats par planète, et après chaque deuxième combat, une nouvelle planète est débloquée jusqu’à atteindre la redoutable Étoile de la Mort. Pour finir l’aventure, l’utilisateur devra choisir entre un duel final contre Leia ou Dark Vador... Et soyons honnêtes, que vous finissiez par épouser l’un ou l’autre, les réunions de famille risquent d'être assez spatiales !",
...
VALUES (
    ...
    'React, NodeJS, ExpressJS, MySQL',
    "Dans le cadre du projet Film Paradise, nous avons adopté les méthodologies Scrum et Agile pour optimiser notre processus de développement. Lubin Chauvreau a exercé les fonctions de Scrum Master, tandis que Johnny Domingues a occupé le poste de Product Owner. Les autres membres de l'équipe ont contribué en tant que développeurs.
Nous avons élaboré un backlog via Google Sheets, qui a également servi d’outil principal pour notre gestion de projet. Par ailleurs, nous avons conçu une maquette sur Figma pour nous permettre de visualiser notre concept avant de commencer le développement. Étant donné qu'il s'agissait de notre premier projet en React et que c'était également notre première expérience avec le développement backend, nous n'avons pas réalisé de modélisation de base de données.",
    "Film Paradise est une plateforme de e-commerce dédiée aux films numériques, offrant aux utilisateurs une expérience de navigation fluide et intuitive. Les utilisateurs peuvent explorer une liste complète de films, filtrables par catégories ou via une barre de recherche dédiée.
